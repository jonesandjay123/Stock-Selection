...
# 修改 get_data 函數以使用 Quandl API
def get_data(symbol: str) -> dict:
    start_date = "2021-01-01"
    end_date = "2021-12-31"
    url = f"https://data.nasdaq.com/api/datatables/SHARADAR/SF1?ticker={symbol}&dimension=MRY&start_date={start_date}&end_date={end_date}&api_key={API_KEY}"
    r = requests.get(url)
    try:
        data = r.json()
...
def calculate_indicators(data: dict) -> dict:
    ...
    # 使用新的資料格式提取收盤價
    close_prices = [row[3] for row in data["datatable"]["data"]]
