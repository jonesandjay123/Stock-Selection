...
def calculate_indicators(stock_data):
    ...
    macd, macdsignal, macdhist = talib.MACD(
        stock_data["adjClose"], fastperiod=12, slowperiod=26, signalperiod=9)
    ...
    bb_upper, bb_middle, bb_lower = talib.BBANDS(
        stock_data["adjClose"], timeperiod=20)
    bb_upper, bb_middle, bb_lower = bb_upper.dropna(
...
def get_top_N_stocks(stock_list, N, indicator_display_amount, cache_folder_name):
    ...
    for stock_symbol in stock_list:
        try:
            # 獲取股票歷史數據
            stock_data, indicators, latest_date_data = get_stock_data(
                stock_symbol, start_date, end_date, cache_folder_name)
            stock_df = pd.DataFrame(stock_data)
            ...
            stock_results.append({
                ...
                'indicators': {k: v[-indicator_display_amount:].tolist() if isinstance(v, pd.Series) else v for k, v in indicators.items()},
            })
    ...
    # 根據score序排序，取前N名
    top_N_stocks = sorted(
        stock_results, key=lambda x: x['score'], reverse=True)[:N]
...
def main():
    ...
    top_N_stocks = get_top_N_stocks(
        stock_list, N, indicator_display_amount, cache_folder_name)  # 取得前N名股票
    top_N_stocks_json = json.dumps(top_N_stocks, indent=2)  # 將結果轉為JSON格式

    # 輸出結果
    simplified_top_N_stocks = [
        {"symbol": stock["symbol"], "score": stock["score"],
...
if __name__ == "__main__":
    ...
    # 可根據需要自定義股票清單
    stock_list = Helper.load_stock_list('voo_stock_list.txt')[
        :80]  # 讀取整個voo_stock_list.txt中的200支股票
